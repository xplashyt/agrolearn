...
//
// Sin imports (ni de Node ni de Wompi): así el checkout del navegador y las
// rutas de API usan exactamente el mismo criterio para decidir qué mensaje
// mostrarle a quien está pagando. Mismo patrón que lib/wompi-env.ts: módulo
// isomorfo, sin dependencias.
//
...
//    lo observamos directamente en la respuesta del fetch. Un 403 (o una
...
//    literal de "IP baneada", pero esa suele ser la causa real detrás de ese
//    código—.
...
// Clasifica una falla al CREAR el pago (tokenización o /transactions), es
// decir, antes de que exista una transacción con estado. A diferencia del
// rechazo del emisor, esto sí se puede determinar con certeza porque lo
// observamos directamente en la respuesta HTTP.
